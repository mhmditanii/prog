...
class App(tk.Tk):
    ...
    def refresh_book_var(self):
        ...
        self.book_var.set([book["title"] for book in library])
        self.total_books_label.config(text=f"Total Books: {len(library)}")

    def refresh_search_results(self, search_variable):
        library = controller.book_list_search(search_variable)
        self.book_var.set(library)
        self.total_books_label.config(text=f"Total Books: {len(library)}")
    ...
    def generate_book(self, index):
        book = [f"Book{index}", f"Author{index}", 2000]
    ...
    def start_process(self):
        for i in range(1000000):
            if self.stop_flag:
                break
            self.generate_book(i)
            if i % 100 == 0:
                self.refresh_book_var()

    def start_generate_button(self):
    ...
    def __init__(self):
        ...
        filemenu.add_command(label='New', command=lambda: model.create_file())
        filemenu.add_command(label='Open', command=model.update_file_path)
        ...
        self.generate_button = tk.Button(left_frame, text="Generate 1 Million Book Entries", command=lambda: self.start_generate_button() )
        self.generate_button.pack(pady=5)
        self.stop_button = tk.Button(left_frame, text="Stop Generation", command=self.end_generation)
...
###########################################################################
        ...
        self.library = model.library
        self.book_var = StringVar(value=[book["title"] for book in self.library])
        ...
        self.total_books_label = tk.Label(buttons_frame, text=f"Total Books: {len(self.library)}")
